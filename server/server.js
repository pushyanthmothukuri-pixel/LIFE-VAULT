...
app.use(cors({
  origin: 'http://localhost:5173', // Vite client dev port
...
app.post('/api/auth/register', async (req, res) => {
  ...
  try {
    ...
    Database.createUser(username, dbPasswordHash, salt, recoveryPayload, recoveryKeyHash);
...
app.post('/api/auth/recovery-payload', (req, res) => {
  const { username } = req.body;
  if (!username) {
    return res.status(400).json({ error: 'Username is required' });
  }
...
app.post('/api/auth/reset-password-with-recovery', async (req, res) => {
  ...
  try {
    ...
    Database.resetPassword(username, dbPasswordHash, newSalt, newRecoveryPayload, newRecoveryKeyHash, newVaultData);
...
app.get('/api/vault', authenticateToken, (req, res) => {
  try {
    const user = Database.getUser(req.user.username);
    if (!user) {
      return res.status(404).json({ error: 'User not found' });
    }
...
app.post('/api/vault', authenticateToken, (req, res) => {
  ...
  try {
    Database.updateVault(req.user.username, vaultData);
